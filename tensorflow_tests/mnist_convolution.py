...
def run_shifted_session(shift):
    ...
    run = accuracy.eval(feed_dict={x: shifted, y_: mnist.test.labels, keep_prob: 1.0})
    return run
...
with tf.Session() as sess:
    saver.restore(sess, './export/cnn-model.ckpt-19000')
    ...
    #     [imresize(img.reshape(28, 28), 100 * SCALE, interp='cubic').reshape(28 ** 2 * SCALE ** 2) for img
    #      in mnist.test.images])
    #
    # print('accuracy scaled %g' % accuracy.eval(feed_dict={x: scaled_images, y_: mnist.test.labels, keep_prob: 1.0}))
    ...
    # plot_results(_x, _y, _z1, None, None)
...
#         if i % 1000 == 0:
#             save_path = saver.save(sess, "./export/cnn-model.ckpt", global_step=i)
